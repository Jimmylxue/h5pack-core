	...
	const r = new (class {
		...
		callNative(e, t, a) {
			return new Promise((r, s) => {
				...
				this.callbacks.set(i, { resolve: r, reject: s, timeoutId: o })
				const n = {
					type: 'bridge_call',
					callId: i,
					module: e,
		...
		handleNativeMessage = e => {
			try {
				...
				if ('bridge_response' === t.type) {
					const { callId: e, success: a, data: r, error: s } = t,
						i = this.callbacks.get(e)
					i &&
						(this.cleanupCallback(e),
						a ? i.resolve(r) : i.reject(new Error(s || 'Native call failed')))
				}
			} catch (e) {
